...
export function PracticeSession({ weekNumber, words: initialWords, language = "zh", handwritingMode = false }: PracticeSessionProps) {
    ...
    // Apple Pencil persistence
    const [onlyPen, setOnlyPen] = useState(false);
    useEffect(() => {
        const saved = localStorage.getItem("tingxie-use-apple-pencil");
        if (saved !== null) {
            setOnlyPen(saved === "true");
        }
    }, []);
    ...
    const toggleOnlyPen = () => {
        const next = !onlyPen;
        setOnlyPen(next);
        localStorage.setItem("tingxie-use-apple-pencil", String(next));
    };
    ...
    return (
        <div className="max-w-xl mx-auto p-4 md:p-8 min-h-screen flex flex-col select-none" style={{ WebkitTouchCallout: 'none', WebkitUserSelect: 'none' }}>
